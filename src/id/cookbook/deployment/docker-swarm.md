...
This guide walks you through deploying a Yii application to Docker Swarm
from a blank server, using Caddy as a reverse proxy and a container registry
(Forgejo or Gitea).
...
Create a dedicated overlay network for Caddy to communicate with your
services:
...
The Yii application template includes Caddy labels by default. Deploy Caddy
...
If using Traefik, you'll need to modify the Yii application's
...
If Caddy/Traefik can't obtain certificates: - Verify DNS is pointing to your
...
`docker -H ssh://docker-web service logs caddy` or `traefik_traefik`
...
container registry (Forgejo or Gitea)  - Automatic HTTPS via Caddy or
Traefik - Zero-downtime deployments with rolling updates - High availability
...
Proxy](https://github.com/lucaslorentz/caddy-docker-proxy)  - [Traefik
