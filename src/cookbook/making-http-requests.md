...
The package installs the PSR HTTP interfaces and `guzzlehttp/psr7`, which provides PSR-7 messages and PSR-17 factories.
...
Use `ClientInterface` for sending requests and PSR-17 factories for creating requests, streams, and URIs.

## Send a GET request

Inject `Psr\Http\Client\ClientInterface` and `Psr\Http\Message\RequestFactoryInterface` into the service that talks to
the external API:
...
PSR-18 clients throw `ClientExceptionInterface` for request sending errors. HTTP status codes such as `404` and `500`
are valid responses, so check the status code in your service.
...
Depend on PSR interfaces for regular application services. Use `GuzzleHttp\Client` directly in infrastructure code that
needs Guzzle-only features such as asynchronous requests, handler stacks, or middleware.
