...
starting from a blank server, using [Traefik](https://traefik.io/traefik) as
...
After deployment, access Forgejo at `https://git.example.com` and complete the initial setup.
...
When using Traefik, you'll need to modify the Yii application's `docker/prod/compose.yml` to use Traefik labels
...
- Check logs: `docker -H ssh://docker-web service logs traefik_traefik`
