...
When the application works with several databases, keep `ConnectionInterface` for the primary connection and add a
class alias for each named connection:
...
The alias class names are used as service IDs and type hints. They don't need PHP files of their own.
...
Services that need the primary database can still depend on `ConnectionInterface`. Services that need another database
should type-hint the corresponding alias:
...
In this repository there are both methods to get data and `save()` to do insert or update. DB returns raw data
as arrays but our repository automatically creates entities from this raw data so later we operate typed data.
...
Note that we've grouped all page-related routes with a group under `/pages` prefix. That is a convenient way to both not
to repeat yourself and add some extra middleware, such as authentication, to the whole group.
